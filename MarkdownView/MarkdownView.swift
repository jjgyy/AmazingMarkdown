...
open class MarkdownView: UIView {
    ...
    var estimatedProgress: Double {
        get {
            return self.webView?.estimatedProgress ?? 0.0
        }
    }
    ...
    public func load(markdown: String?, enableImage: Bool = true, completion: @escaping () -> Void ) {
        self.loadingCompletion = completion;
        self.load(markdown: markdown, enableImage: enableImage)
    }
...
extension MarkdownView: WKNavigationDelegate {
    ...
    public func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        ...
        if let completion = loadingCompletion {
